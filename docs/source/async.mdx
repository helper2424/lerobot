...
python -m lerobot.async_inference.robot_client \
    ...
    --wandb.enable=True # WANDB: enable wandb logging
    --wandb.project=lerobot-async-inference # WANDB: wandb project name
    --wandb.entity=your_entity # WANDB: wandb entity (optional)
```
...
- `CLIENT`: the threshold for the chunk size before sending a new observation to the server, and the function to aggregate actions on overlapping portions. Optionally, you can also log the queue size to Weights & Biases at runtime, to help you tune the `CLIENT` parameters.
...
- `wandb.enable` enables logging to Weights & Biases, which is a useful tool to tune the `CLIENT` parameters.
...
from lerobot.async_inference.helpers import log_action_queue_sizes_to_wandb
...
# 2. Create wandb configuration (optional)
wandb_cfg = WandBConfig(
    enable=True,
    project="lerobot-async-inference",
    entity="your_entity"  # optional
)
...
if client.start():
    ...
    try:
        ...
        # Queue sizes are automatically logged to wandb if wandb is enabled
        # (handled in the client's shutdown process)
```
<!-- prettier-ignore-end -->
...
3. **Adjust `chunk_size_threshold`**.
   ...
   - We found values around 0.5-0.6 to work well. If you want to tweak this, spin up a `RobotClient` with wandb enabled (`--wandb.enable=True`). This will log the action queue size evolution to Weights & Biases, and you can use it to find the value of `chunk_size_threshold` that works best for your setup.
...
<p align="center">
  <i>
    The action queue size is logged to Weights & Biases when wandb is enabled
    (`--wandb.enable=True`), for various levels of `chunk_size_threshold` (`g`
    in the SmolVLA paper).
  </i>
</p>
