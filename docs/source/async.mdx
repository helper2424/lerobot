...
python -m lerobot.async_inference.robot_client \
    ...
    --debug_log_queue_size_to_wandb=True # CLIENT: whether to log the queue size to Weights & Biases
```
...
- `debug_log_queue_size_to_wandb` is a useful tool to tune the `CLIENT` parameters.
...
if client.start():
    ...
    try:
        ...
        # (Optionally) log the action queue size to wandb
...
3. **Adjust `chunk_size_threshold`**.
   ...
   - We found values around 0.5-0.6 to work well. If you want to tweak this, spin up a `RobotClient` setting the `--debug_log_queue_size_to_wandb` to `True`. This will log the action queue size evolution to Weights & Biases, and you can use it to find the value of `chunk_size_threshold` that works best for your setup.
...
<p align="center">
  <i>
    The action queue size is logged to Weights & Biases when the
    `--debug_log_queue_size_to_wandb` flag is passed, for various levels of
